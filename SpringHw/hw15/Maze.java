// Team SomethingElse - Derek Song, Sean Roudnitsky
// APCS2 - pd8
 ...
 *  The hero would test all the base cases and see if he needs to diverge from
 *  the recursive function. In the recursive calls, the hero continues going 
 *  up, then  if there is no path afterwards, it tests going down, then left, 
 *  and finally right. Each time, testing the base cases and if none are true, 
 *  then it continues.
 ***/
...
class MazeSolver 
{
	...
	//other base case(s)...
	    ...
	    return;
		  ...
		  || y >= maze[0].length || y < maze[0].length) { // v o i d
	    return;
	}
	//recursive reduction
	else {
	    maze[x][y] = '@'; // marks thy path
